...
| `current_participants` | MAY | 直接視聴者数 | `numl`(負値は省略。受信側はタグ省略を「不明 = -1」として復元する — contracts/http-yp.md) |
...
| `["peca","relays","<count>"]` | リレー数 | `numr`(負値は省略。受信側はタグ省略を「不明 = -1」として復元する) |
| `["peca","track","<title>","<artist>","<album>","<url>"]` | 曲情報(空要素可) | `titl`/`crea`/`albm`。`<url>` 要素は **v1 では常に空文字列**(対応する PCP atom を受信しない — contracts/pcp-announce.md) |
...
3. **kind/タグ形式**: kind=30311、`d` は hex 32 桁、`status` ∈ {live, ended}、タグ数 ≤ 64、各タグ要素長 ≤ 1024 バイト(違反は `event_invalid_format`)
4. **時刻**: `created_at` が現在+`max_clock_skew_sec`(既定 300 — data-model §Settings が単一出典)秒を超える未来なら破棄(`event_time_skew`)。**本検査で拒否するのは未来方向のみ**であり、過去方向のずれは鮮度窓(600 秒)・`expiration` 判定により自然減衰する(遅れ 300〜600 秒は短寿命で受理、600 秒超は実質不可視 — spec Edge Case「時計のずれ」)。鮮度窓 600 秒・再発行周期 60 秒の設計により、±300 秒未満のずれでは鮮度・`expiration` 判定の誤除去は生じない。鮮度・期限の判定はすべて受信者ローカル時計を基準に行う
5. **内容**: 数値タグの範囲(bitrate ≤ 100000 等)、`tip` の ip:port 形式、文字列は制御文字除去(違反は `event_invalid_format`)
6. **PoW(任意)**: `min_pow_bits` > 0 のとき、NIP-13 のコミット難易度を満たさないイベントは破棄(`event_pow_insufficient`)
