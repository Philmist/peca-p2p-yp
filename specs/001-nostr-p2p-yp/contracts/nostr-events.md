# Contract: nostr イベント(チャンネル掲載・実況)

**Status**: v1 対象は kind 30311 のみ。kind 1311 は将来フェーズの予約定義。

## kind 30311 — チャンネル掲載イベント(NIP-53 Live Streaming Event)

addressable イベント。リレー上では `(30311, pubkey, d)` で旧版が置換される。
...
| `relays` | MAY | 推奨リレー URL 群 | 自ノードの write リレー |
| `expiration` | MUST | created_at + 600(NIP-40) | 鮮度管理(research R2) |
...
- 配信終了(playing=false / PCP 切断)時に `status=ended` で最終発行(補助として NIP-09 削除要求を併用してよい)
- 署名鍵は当該チャンネルに割り当てたペルソナのもの。**他ペルソナの情報(label 等)をイベントに含めてはならない(FR-013)**

### 受信検証(購読側)— Principle II: trust nothing from the network

順序どおりに検査し、失敗したら破棄してセキュリティイベントに記録する:

1. **サイズ**: 直列化イベント全体 ≤ 16KB(超過は `nostr_oversize`)
2. **署名**: nostr-sdk による id・sig 検証(失敗は `nostr_invalid_sig` — FR-005)
