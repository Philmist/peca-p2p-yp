...
- [ ] T063 スレ開設の配信者操作を実効化する: 変更系エンドポイント(スレ開設 — 掲載中チャンネルのペルソナ限定・X-Api-Token 保護)を src/web/livechat.rs に追加し、src/main.rs で LivechatRegistry(runtime.livechat())と配線して `open_thread` の本番呼び出し経路を作る(現状は呼び出し元がテストのみで、稼働バイナリでスレを開設できない) per FR-001 / US1-AC1 (missing) — CRITICAL
- [ ] T064 参加者セッションの常駐管理を実装する: 「スレを開く」操作(join_thread スタブの実装置換)を起点に `livechat::participant::run_forever` を駆動するセッションマネージャを実装し、確定レスの閲覧供給・凍結/復帰・クローズ揮発・バックオフ再接続を稼働バイナリで成立させる — src/livechat/(マネージャ新設)+ src/web/livechat.rs + src/main.rs per US1-AC2〜AC4 / FR-004 / FR-010 (missing) — CRITICAL
- [ ] T065 LivechatDirectory を配線する: 自板(LivechatRegistry)・他ノード板(EventStore の kind 31311 announce + T064 の参加者セッション)を束ねる実装を src/main.rs で生成し `.with_livechat_directory(...)` へ渡す(main.rs の TODO 解消)。これによりスレ一覧・詳細・next・close の各 API を実効化する(現状は一覧が常に空・詳細/next/close が常に 404) per US1-AC1 / FR-013 / FR-022 (partial) — CRITICAL
- [ ] T066 書き込みのローカル API 経路を追加する: T064 のセッション経由で `compose_write` → RES 送信を行う変更系エンドポイントを src/web/livechat.rs に追加し、自分の未確定投稿の「送信中」区別表示を応答に含める(現状、稼働バイナリで書き込めるのは自板への bbs.cgi のみ) per US2-AC1 / FR-008 (missing)
- [ ] T067 NG/BAN/ConnBan/板鍵ローテーションの操作 API を追加する: src/web/livechat.rs に変更系エンドポイントを追加し moderation/board ドメイン層と配線する(web/livechat.rs のモジュール doc に「未追加」と明記されている宿題。BAN は採番拒否・ConnBan は接続拒否・ローテーションは初回 PoW 前提の再生成) per FR-017 / FR-019 / FR-020 (missing)
- [ ] T068 板設定変更の操作 API を追加する: `LivechatRegistry::update_settings`(検証・SETTINGS 即時配布は実装済み)を呼ぶ変更系エンドポイントを src/web/livechat.rs に追加する per FR-022 / US1-AC5 (missing)
