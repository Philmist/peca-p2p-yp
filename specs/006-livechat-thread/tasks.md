...
**Purpose**: /speckit-converge(2026-07-14)の所見。ドメイン層・契約テスト・cucumber は
完成済みだが、稼働バイナリへの配線(操作 API・UI)が未達。CRITICAL(T063〜T065)は
P1 ストーリーの製品上の成立を塞いでいるギャップ。T064 は T066/T069 の前提、
T063/T065 は他の操作系の前提となる。

- [X] T063 スレ開設の配信者操作を実効化する: 変更系エンドポイント(スレ開設 — 掲載中チャンネルのペルソナ限定・X-Api-Token 保護)を src/web/livechat.rs に追加し、src/main.rs で LivechatRegistry(runtime.livechat())と配線して `open_thread` の本番呼び出し経路を作る(現状は呼び出し元がテストのみで、稼働バイナリでスレを開設できない) per FR-001 / US1-AC1 (missing) — CRITICAL
- [X] T064 参加者セッションの常駐管理を実装する: 「スレを開く」操作(join_thread スタブの実装置換)を起点に `livechat::participant::run_forever` を駆動するセッションマネージャを実装し、確定レスの閲覧供給・凍結/復帰・クローズ揮発・バックオフ再接続を稼働バイナリで成立させる — src/livechat/(マネージャ新設)+ src/web/livechat.rs + src/main.rs per US1-AC2〜AC4 / FR-004 / FR-010 (missing) — CRITICAL 【実装: `livechat::participant::run_session`(ライブ状態共有 + 書き込み多重化・キャンセル安全)+ `livechat::manager::ParticipantManager`。join/leave API 実効化。実ホストへの end-to-end 統合テストで検証】
- [X] T065 LivechatDirectory を配線する: 自板(LivechatRegistry)・他ノード板(EventStore の kind 31311 announce + T064 の参加者セッション)を束ねる実装を src/main.rs で生成し `.with_livechat_directory(...)` へ渡す(main.rs の TODO 解消)。これによりスレ一覧・詳細・next・close の各 API を実効化する(現状は一覧が常に空・詳細/next/close が常に 404) per US1-AC1 / FR-013 / FR-022 (partial) — CRITICAL 【実装: `LivechatAdapter`。一覧は自板 + 他ノード板 announce、自板の詳細/next/close は実効化。他ノード板の詳細(確定レス)は T064 の参加者セッションが供給する(T069 と併せて配線)】
- [X] T066 書き込みのローカル API 経路を追加する: T064 のセッション経由で `compose_write` → RES 送信を行う変更系エンドポイントを src/web/livechat.rs に追加し、自分の未確定投稿の「送信中」区別表示を応答に含める(現状、稼働バイナリで書き込めるのは自板への bbs.cgi のみ) per US2-AC1 / FR-008 (missing) 【実装: `POST .../{board_id}/write`。送信中(pending)は `GET .../{board_id}` の `pending` 配列で観測。初回 PoW はマネージャが板鍵ごとに判定】
- [X] T067 NG/BAN/ConnBan/板鍵ローテーションの操作 API を追加する: src/web/livechat.rs に変更系エンドポイントを追加し moderation/board ドメイン層と配線する(web/livechat.rs のモジュール doc に「未追加」と明記されている宿題。BAN は採番拒否・ConnBan は接続拒否・ローテーションは初回 PoW 前提の再生成) per FR-017 / FR-019 / FR-020 (missing)
...
- [ ] T069 互換 API を接続中・凍結中のリモート板に対応させる: T064 の参加者セッションが保持する確定レス・板設定を subject.txt / dat / SETTING.TXT / head.txt / bbs.cgi(書き込みはセッション経由の RES 送信)へ供給する(contracts/compat-api.md §板の URL 対応「接続中・凍結中のスレを持つ板のみが解決される」。現状は自ノードホスト板限定 — Phase 8 コミットで受容した既知の逸脱) per US6-AC1 / FR-026 / FR-028 (partial)
- [ ] T070 Web UI に実況スレ画面を追加する: スレ一覧(announce 由来)・スレを開く・閲覧・書き込み・板設定・NG/BAN・ローテーション・次スレ/クローズ・板 URL コピー表示を ui/ に実装し既存ページ群と同じ流儀で配線する(現状 ui/ に livechat ページが存在しない) per US1-AC2/AC5・US2-AC1(操作面) (partial)
