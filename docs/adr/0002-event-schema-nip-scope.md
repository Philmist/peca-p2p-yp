...
2. **実機検証結果**: **成立を確認**(2026-07-04〜05、T058)。無改造の PeerCastStation
   現行安定版が、index.txt の TIP のみで視聴を開始できることを IPv4・IPv6
   (`[addr]:port` ブラケット形式)の双方で確認した。既知の制約: TIP はグローバル
   IP のため、配信者と同一 LAN 内からの視聴はルーターの NAT ループバック
   (ヘアピン NAT)対応に依存する(検証環境のルーターは対応)。firewalled
   (PUSH フラグ)チャンネルは仕様どおり TIP 空で直接視聴不可
