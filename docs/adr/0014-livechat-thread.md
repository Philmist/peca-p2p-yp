...
- **実機確認済み(2026-07-14 / T062)**: 利用者所有の実況ツールで SC-007 を実施した。
  ただし **現行の PC 用専ブラで本互換 API と互換動作するものはほぼ存在せず**、多くは
  ローカルサーバー(`127.0.0.1`)の板 URL を認識できず無視する。JaneXeno のみ
  `http://127.0.0.1:7183/<board_id>/` へ誘導できたが、SETTING.TXT のキー突合確認は
  UI 上困難・head.txt 表示は不可能で、この 2 観点は現行専ブラでは検証しきれない。
  残る 6 観点(hex64 板名の登録可否・SJIS/数値文字参照描画・Range/304 の噛み合い・dat
  落ち解釈・確認画面なし書き込み・固定 ID の NG 機能)は確認済み・成立。
  **SJIS 既定の主仮説は本確認の範囲で反証されておらず維持する。**
- **判断**: 「契約テストで裏付けた実装 + 実機で 6/8 観点を確証した仮説」であり、
  未検証の 2 観点(SETTING.TXT / head.txt)は現行専ブラの制約に起因し実装側の不成立では
  ないため追加タスク化しない。T062 は完了。実機検証の結果は
  specs/006-livechat-thread/research.md R5 実機確認結果へ追記した。専ブラのローカル板
  URL 誘導挙動を前提とした追加要件は本 006 のスコープ外とし別 spec へ切り出す。
