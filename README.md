...
中央サーバーに依存しない純粋 P2P で置き換える Windows 向けソフトウェアです。
...
- 秘密鍵は端末内に暗号化(Windows DPAPI)して保管されます。バックアップが必要な場合は
...
利用者は配布 exe のみで動作します。ソースからビルドする場合:

```powershell
cargo build --release   # 生成物: target\release\peca-p2p-yp.exe
