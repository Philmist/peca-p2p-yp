...
> 別の端末から操作画面を開きたい場合は
> [SSH ポート転送](#別の端末から操作画面を開くssh-ポート転送)を利用してください。
...
### 別の端末から操作画面を開く(SSH ポート転送)

操作用 Web UI(`7180`)は安全のため loopback のみを待ち受けるため、サービスとして
無人稼働させているマシンでは、そのマシン上のブラウザからしか開けません。
同じネットワークの別の端末から操作したい場合は、**SSH ポート転送**を利用してください。
本ソフトウェア側の設定変更は不要で、経路は SSH により暗号化されます。

手元(操作したい端末)で次を実行します:

```bash
ssh -N -L 7180:127.0.0.1:7180 <ユーザー名>@<サーバーのホスト名または IP>
```

このコマンドを実行している間、手元のブラウザで `http://127.0.0.1:7180/` を開くと
サーバー上の操作画面をそのまま利用できます(`-N` はトンネル専用でシェルを開かない指定です)。
`index.txt` も同じトンネル経由で取得できます。手元の PeerCastStation から掲載したい場合は
`-L 7146:127.0.0.1:7146` を追加すれば PCP 受信も転送できます。

> LAN 内の他端末へ HTTP/PCP ポートを直接公開する機能は、設計判断
> ([ADR-0006](./docs/adr/0006-unencrypted-transport.md) 決定 4)により現在提供していません。
> 平文 HTTP のままの公開は API トークンの盗聴を許すため、暗号化された SSH 転送を推奨します。
